...
const $=(s,r=document)=>r.querySelector(s),$$=(s,r=document)=>[...r.querySelectorAll(s)];
  ...
  if(grid){
	    grid.innerHTML=available.length?available.map(([id,b],i)=>`<article class="book-card reveal ${i%3===1?'reveal-delay':i%3===2?'reveal-delay-2':''}" data-book-card="${escapeHtml(id)}">
	      <div class="book-image"><img src="${escapeHtml(b.image||'')}" alt="${escapeHtml(b.title||'Book')}" loading="lazy" onerror="this.onerror=null;this.src='assets/coming-soon-cover.webp?v=20260925';this.classList.add('asset-fallback')"><span class="book-badge"><i data-lucide="check"></i> Available</span></div>
